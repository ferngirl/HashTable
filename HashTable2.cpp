...
private:
	void DetectedLine(size_t& index)//线性探测
	{
		index++;//查找后面的合适位置
	...
	void Detected2(size_t& index,size_t& i)//二次探测
	{
		index = index + 2 * i + 1;
		i++;
		if (index >= _hash.size())
	...
	void CheckTable()
	{
		...
		{
			//搬移元素，重新计算地址
			HashTable<k,v,_HashFun<k>,IsLine> hs(GetNextPrime(_hash.size()));// 一个新的哈希表
